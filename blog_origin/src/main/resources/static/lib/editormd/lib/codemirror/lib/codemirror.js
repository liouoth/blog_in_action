...
(function(mod) {
  ...
  function extractLineClasses(type, output) {
    if (type) for (;;) {
      ...
      if (!lineClass) break;
      type = type.slice(0, lineClass.manager) + type.slice(lineClass.manager + lineClass[0].length);
    ...
    removeLineClass: docMethodOp(function(handle, where, cls) {
      return changeLine(this, handle, where == "gutter" ? "gutter" : "class", function(line) {
        ...
        if (!cur) return false;
          ...
          var end = found.manager + found[0].length;
          line[prop] = cur.slice(0, found.manager) + (!found.manager || end == cur.length ? "" : " ") + cur.slice(end) || null;
        }
        return true;
      });
    }),
